...
namespace IDZ_szi
{
    public partial class Complex : Form
    {
        ...
        private void FillComplexComboBox()
        {
            ...
            using (SqlDataReader reader = command.ExecuteReader())
            {
                while (reader.Read())
                {
                    ...
                    comboBox15.Items.Add(employeeName);
                }
            }
        }
        ...
        private void FillRequestDatesComboBox()
        {
            ...
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    // Получение списка заявок, которые больше 24 мая и нет в таблице "Оплата"
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    string insertRequestQuery = "INSERT INTO Request (Date_Request, BuyerID) " +
                        "VALUES (@DateRequest, (SELECT ID_Buyer FROM Buyer WHERE Surname_Buyer = @Surname AND Name_Buyer = @Name and Patronymic_Buyer = @Patronymic))";
                    SqlCommand insertRequestCommand = new SqlCommand(insertRequestQuery, connection);
                    ...
                    // Получение ID комплексного решения на основе названия
                    string selectComplexIDQuery = "SELECT ID_Complex FROM [Complex solution] WHERE Title_Complex = @ComplexName";
                    SqlCommand selectComplexIDCommand = new SqlCommand(selectComplexIDQuery, connection);
                    selectComplexIDCommand.Parameters.AddWithValue("@ComplexName", selectedComboBox15Value);
                    int complexID = Convert.ToInt32(selectComplexIDCommand.ExecuteScalar());

                    // Вставка данных в таблицу "[Complex and Request]"
                    string insertRequestComplexQuery = "INSERT INTO [Complex and Request] (RequestID, ComplexxID, Сount_Complex) " +
                        "VALUES (@RequestID, @ComplexID, @Count)";
                    SqlCommand insertRequestComplexCommand = new SqlCommand(insertRequestComplexQuery, connection);
                    insertRequestComplexCommand.Parameters.AddWithValue("@RequestID", lastRequestID);
                    insertRequestComplexCommand.Parameters.AddWithValue("@ComplexID", complexID);
                    insertRequestComplexCommand.Parameters.AddWithValue("@Count", selectedCount);
                    insertRequestComplexCommand.ExecuteNonQuery();
        ...
        private void Complex_FormClosing(object sender, FormClosingEventArgs e)
        {
